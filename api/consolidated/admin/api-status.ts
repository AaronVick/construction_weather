...
try {
  ...
  if (!admin.apps || admin.apps.length === 0) {
    console.log('Initializing Firebase Admin in consolidated API status endpoint');
    ...
    console.log('Firebase Admin already initialized in consolidated endpoint');
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!db || !auth) {
      console.error('Firebase services not available in consolidated endpoint');
