...
import { WeatherTestFormData, JobsiteOption } from './types';
  ...
  return (
    <Grid item xs={12}>
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select {...field} label="Location Type">
              ...
              <MenuItem value="jobsite">Jobsite</MenuItem>
            </Select>
          )}
        />
      </FormControl>
      ...
      {locationType === 'jobsite' && (
        <FormControl fullWidth error={!!errors.jobsiteId}>
          <InputLabel>Jobsite</InputLabel>
          <Controller
            name="jobsiteId"
            control={control}
            rules={{ required: 'Jobsite is required' }}
            render={({ field }) => (
              <Select {...field} label="Jobsite">
                {jobsites.map(jobsite => (
                  <MenuItem key={jobsite.id} value={jobsite.id}>
                    {jobsite.name} {jobsite.address && `(${jobsite.address})`}
                  </MenuItem>
                ))}
              </Select>
            )}
          />
          {errors.jobsiteId && (
            <FormHelperText>{errors.jobsiteId.message}</FormHelperText>
          )}
        </FormControl>
      )}
    </Grid>
